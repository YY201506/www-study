...
$attachDir='../../' . $UploadEditorPath;//上传文件保存路径，结尾不要带/
$dirType=1;//1:按天存入目录 2:按月存入目录 3:按扩展名存目录  建议使用按天存
//$maxAttachSize=2097152;//最大上传大小，默认是2M
$maxAttachSize=$UploadEditorSize * 1024;
//$upExt='txt,rar,zip,jpg,jpeg,gif,png,swf,wmv,avi,wma,mp3,mid,doc,docx,xls,xlsx';//上传扩展名
$upExt=$upLinkExt.','.$upImgExt.','.$upFlashExt.','.$upMediaExt;
	...
	if(preg_match('/^('.str_replace(',','|',$upExt).')$/i',$extension))
	{
		...
		else
		{
			...
			//else $msg="{'url':'".$targetPath."','localname':'".jsonString($localName)."','id':'1'}";//id参数固定不变，仅供演示，实际项目中可以是数据库ID
				...
				$targetPathReturn = str_replace("..\/","",(string)$targetPath);
				$targetPathReturn = "/" . $targetPathReturn;
				$msg="{'url':'".$targetPathReturn."','localname':'".jsonString($localName)."','id':'1'}";//id参数固定不变，仅供演示，实际项目中可以是数据库ID
			}
		}
	}
